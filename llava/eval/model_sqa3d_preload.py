...
def eval_model(questions, args):
    ...
    for line in tqdm(questions):
        ...
        if args.feature_dir:
            ...
            feature_path = os.path.join(args.feature_dir, f"{video_id}.pt")
            if not os.path.exists(feature_path):
                # Try alternative path pattern
                feature_path = video_path.replace('.mp4', '.pt').replace('videos', 'spatial_features')
            ...
            if os.path.exists(feature_path):
                ...
                if isinstance(spatial_features, dict) and 'spatial_features' in spatial_features:
        ...
        if args.extra_prompt:
            question = DEFAULT_IMAGE_TOKEN + f"{args.extra_prompt}\n{time_instruction}\n{qs}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature-dir", type=str, default=None,
                       help="Directory containing precomputed spatial features")
    ...
    parser.add_argument("--answer-file", type=str, default="./llava3d_sqa3d_val_answer_pred.jsonl")
    ...
    if os.path.exists(args.answer_file):
        print(f"The {args.answer_file} already exists! Appending to it.")
